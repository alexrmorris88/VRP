...
class OrderOptimization:
    ...
    def add_data_orders(self):
        ...
        self.df = self.df.merge(self.distance, how='left', on=['Delivery Location Postal Code', 'Delivery Location Postal Code'])

        self.df['pick distance'] = 0
    
        base_rate_path = self.path + "/base rates/base rates.csv"
        self.base_rate = pd.read_csv(base_rate_path)
        self.df = self.df.merge(self.base_rate, how='left', on=['Delivery Location State/Province', 'Delivery Location State/Province'])
    ...
    def date_filter_orders(self, date_start, date_end):
        ...
        self.df["Calculated Load First Pick Date"] = pd.to_datetime(self.df["Calculated Load First Pick Date"], format='%m-%d-%y %H:%M', errors='ignore')
        ...
        self.df["Shipment Create Date"] = pd.to_datetime(self.df["Shipment Create Date"], format='%m-%d-%y %H:%M', errors='ignore')
    ...
    def order_log_data(self):
        ...
        self.df_log = pd.DataFrame(self.df, columns=['drop distance','Order Number','Delivery Location Name',
                                                     ...
                                                     'Delivery Location Postal Code','Shipment Status','First Pick Actual Time',
    ...
    def tms_orders_print(self):
        ...
        for id in self.df_log['Order Number'].unique():
            ...
            print_solution += '<br>Weight: {:,} lbs | '.format(round(sum(self.df_log['Weight (lb)'][self.df_log['Order Number'] == id])))
            print_solution += 'Distance: {} </br>'.format(sum(self.df_log['drop distance'][self.df_log['Order Number'] == id]))
        ...
        print_solution += '<br></br>'
        print_solution += '<br>Total QC Orders: {} | Total ON Orders: {}</br>'.format(len(self.df_log['Order Number'][self.df_log['Delivery Location State/Province'] == 'QC']), 
                                                                                     len(self.df_log['Order Number'][self.df_log['Delivery Location State/Province'] == 'ON']))
        print_solution += '<br>Total QC Weight: {:,} | Total ON Weight: {:,}</br>'.format(round(sum(self.df_log['Weight (lb)'][self.df_log['Delivery Location State/Province'] == 'QC']),2), 
    ...
    def log_map_orders(self):
        ...
        color = ['red', 'lightgray', 'pink', 'darkred', 'beige', 'orange', 'darkgreen', 'purple', 'lightblue', 'cadetblue',
    ...
    def filter_location_city(self):
        self.opt_data()
        filter_location_city = self.df_opt['Pick-up Location City'] + (', ') + self.df_opt['Pick-up Location State/Province']
        filter_location_city = filter_location_city[:1]
        filter_location_city_2 = self.df_opt['Delivery Location City'] + (', ') + self.df_opt['Delivery Location State/Province']
    ...
    def order_num_dataframe(self):
        ...
        df_order_num_data = pd.merge(df_opt_data, df_log_data,  how='left', left_on=['lat', 'lon'],  right_on=['lat', 'lon']).drop_duplicates().reset_index(drop=True)
    ...
    def order_num_data_link(self):
        ...
        routing_guide_model = pd.DataFrame(order_num_dataframe, columns = ['Order Number', 'lat', 'lon', 'Delivery Location Name'])
        
        order_num_data_link = pd.merge(data_list, routing_guide_model, how='left', left_on=['lat','lon'], right_on=['lat', 'lon']).drop_duplicates().reset_index(drop=True)
    ...
    def print_routing_guide(self, data, manager, routing, solution):
        ...
        order_num_data_link = pd.DataFrame(order_num_data_link, columns = ['Customers', 'lat', 'lon', 'Order Number', 'Delivery Location Name'])
        ...
        for vehicle_id in range(self.data['num_vehicles']):
            ...
            plan_output += '<br>Mode: {} | '.format(
                self.mode(((route_load / pallet_cubic_inches) * self.pallet_weight), route_distance, self.data['drop_state'][node_index]))
            plan_output += 'Total Rate: ${:,} | '.format(round(
                self.opt_rates(((route_load / pallet_cubic_inches) * self.pallet_weight), route_distance,
                self.data['drop_state'][node_index], drop_charges_count), 2))
            plan_output += 'Drops: {} | '.format(drops)
            plan_output += 'Distance: {:,} mi | '.format(route_distance)
            plan_output += 'Weight: {:,} lbs</br>'.format(round((route_load / pallet_cubic_inches) * self.pallet_weight,2))
            ...
            plan_output += 'Remaining Pallets: {}</br>'.format(round(math.floor((truck_cubic_inches - route_load) / pallet_cubic_inches)))
            plan_output += '<br></br>'
            plan_output += '<br></br>'
            print(plan_output)
            opt_total_rate += round(self.opt_rates(((route_load / pallet_cubic_inches) * self.pallet_weight), route_distance,
                                              self.data['drop_state'][node_index], drop_charges_count),2)
        ...
        print_total += 'Truck Space: {} | '.format(math.floor((truck_cubic_inches * self.data['num_vehicles']) / pallet_cubic_inches))
        print_total += 'Space Remaining: {} </br>'.format(math.floor((truck_cubic_inches * self.data['num_vehicles'] - opt_total_load) / pallet_cubic_inches))
        print_total += '<br>_________________________________________________________________________</br>'
        print(print_total)
    ...
    def opt_map_orders(self):
        ...
        opt_color = ['red', 'lightgray', 'pink', 'darkred', 'beige', 'orange', 'darkgreen', 'purple', 'lightblue', 'cadetblue',
                 'black', 'darkblue', 'gray', 'green', 'darkpurple', 'lightred', 'lightgreen']
        ...
        for lt, ln, cm, we, pc, ps in zip(self.pick_lat_fol, self.pick_lon_fol, self.pick_loc_fol, self.pick_weight_fol, self.pick_city_fol, self.pick_state_fol):
            fg3.add_child(folium.Marker(location=[lt, ln],
                                        popup=folium.Popup(f"""<br>Name: <strong>{cm}</strong></br>
                                        <br>Weight: <strong> {we:,}</strong></br>
                                        <br>Location: <strong> {pc}, {ps}</strong></br>""",
    ...
    def routing_guide_orders(self):
        ...
        routing.AddDimensionWithVehicleCapacity(
            ...
            0,  # null slack
            [(self.vehicle_numbers_orders + 1) for i in range(self.data['num_vehicles'])],  # maximum locations per vehicle
            True,  # start cumul to zero
            'num_of_locations')
