    ...
    def log_routing_data(self):
        self.df_log = self.df.groupby(
            ['TMS ID', 'First Pick Actual Date', 'First Pick Actual Time', 'Delivery Location Name',
             ...
             'base rate',
             'Carrier Name', 'Load Total Distance (mi)', 'lat', 'lon'])['Weight (lb)'].sum()
...
# =============================================================================
    ...
    def print_routing_guide(self, data, manager, routing, solution):
        ...
        for vehicle_id in range(self.data['num_vehicles']):
            ...
            while not routing.IsEnd(index):
                ...
                plan_output += '&nbsp; &nbsp; <b>{}</b> ({:,} lbs): '.format(self.data['location_names'][node_index], round(route_load_2))
            ...
            plan_output += 'Weight: {:,} lbs</br>'.format(round((route_load / pallet_cubic_inches) * self.pallet_weight))
            ...
            opt_total_rate += round(self.opt_rates(((route_load / pallet_cubic_inches) * self.pallet_weight), route_distance,
                                              self.data['drop_state'][node_index], drop_charges_count))
    ...
    def opt_map_(self):
        ...
        for lt, ln, vh, cm, we, cl, lo in zip(self.opt_lat_list, self.opt_lon_list, self.vehicle_id_list, self.opt_customers_list,
            ...
            i.add_child(folium.Marker(location=[lt, ln],
                                        popup=folium.Popup(f"""<br>Name: <strong> {cm}</strong></br>
